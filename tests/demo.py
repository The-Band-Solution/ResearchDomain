...
def run_demo():
    ...
    try:
        ...
        ufsc = uni_ctrl.create_university(
            name="Federal University of Santa Catarina", short_name="UFSC"
        )
        florianopolis = campus_ctrl.create_campus(
            name="Florianópolis Campus", organization_id=ufsc.id
        )
        ...
        dr_joyce = researcher_ctrl.create_researcher(
            name="Dr. Joyce", emails=["joyce@ufsc.br"]
        )
        dr_paul = researcher_ctrl.create_researcher(
            name="Dr. Paul", emails=["paul@ufsc.br"]
        )
        ...
        leader_1 = group_ctrl.add_leader(
            team_id=ai_lab.id, person_id=dr_joyce.id, start_date=date(2023, 1, 1)
        )
        leader_2 = group_ctrl.add_leader(
            team_id=ai_lab.id, person_id=dr_paul.id, start_date=date(2024, 1, 1)
        )
