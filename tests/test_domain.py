...
from research_domain.domain.entities.university import Campus, University
...
def test_research_group_many_to_many_knowledge_areas():
    ...
    group = ResearchGroup(name="Advanced Lab", knowledge_areas=[area1, area2])
...
def test_team_member_temporal_leadership():
    ...
    membership = TeamMember(
        person_id=1, team_id=1, role_id=leader_role.id, start_date=start, end_date=end
    )
...
def test_add_researcher_to_group_basic():
    ...
    membership = TeamMember(
        person_id=1, team_id=1, role_id=2, start_date=date.today()  # Researcher Role
    )
...
def test_create_researcher_with_metadata():
    ...
    assert (
        researcher.google_scholar_url == "http://scholar.google.com/citations?user=1234"
    )
...
def test_researcher_many_to_many_knowledge_areas():
    ...
    researcher = Researcher(name="Dr. Metadata", knowledge_areas=[area1, area2])
